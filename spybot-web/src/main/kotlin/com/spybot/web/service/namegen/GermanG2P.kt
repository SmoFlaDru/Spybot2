    ...
    private fun transcribePart(word: String): String {
        ...
        while (i < n) {
            ...
            when {
                ...
                startsWith("tsch") -> { out.append("tʃ"); i += 4 }
                startsWith("tth") -> { out.append("t"); i += 3 }
                startsWith("sch") -> { out.append("ʃ"); i += 3 }
                startsWith("chs") -> { out.append("ks"); i += 3 }
                startsWith("ch") -> {
                    out.append(
                        when {
                            i == 0 -> "k" // Christian, Christoph
                            prevIsBackVowel() -> "x"
                            else -> "ç"
                        },
                    )
                ...
                startsWith("ck") -> { out.append("k"); i += 2 }
                startsWith("ph") -> { out.append("f"); i += 2 }
                startsWith("th") -> { out.append("t"); i += 2 }
                startsWith("dt") -> { out.append("t"); i += 2 }
                startsWith("qu") -> { out.append("kv"); i += 2 }
                startsWith("ng") -> { out.append("ŋ"); i += 2 }
                startsWith("nk") && (i + 2 >= n || isVowel(at(2))) -> { out.append("ŋk"); i += 2 }
                startsWith("sp") && i == 0 -> { out.append("ʃp"); i += 2 }
                startsWith("st") && i == 0 -> { out.append("ʃt"); i += 2 }
                startsWith("ss") -> { out.append("s"); i += 2 }
                startsWith("tz") -> { out.append("ts"); i += 2 }
                ch == 'ß' -> { out.append("s"); i += 1 }
                ch == 'z' -> { out.append("ts"); i += 1 }
                ch == 'x' -> { out.append("ks"); i += 1 }
                ch == 'c' -> { out.append(if (at(1) == 'e' || at(1) == 'i') "ts" else "k"); i += 1 }
                ch == 'v' -> { out.append("f"); i += 1 }
                ch == 'w' -> { out.append("v"); i += 1 }
                ch == 'j' -> { out.append("j"); i += 1 }
                ch == 'y' && (i == 0 || isVowel(at(-1))) && isVowel(at(1)) -> { out.append("j"); i += 1 }
                ch == 's' -> { out.append(if (isVowel(at(1))) "z" else "s"); i += 1 }
                ch == 'h' -> {
                ...
                !isVowel(ch) && at(1) == ch -> { out.append(ch); i += 2 }

                // ---- diphthongs ----
                startsWith("ei") || startsWith("ai") || startsWith("ey") || startsWith("ay") -> { out.append("aɪ"); i += 2 }
                startsWith("eu") || startsWith("äu") || startsWith("oi") || startsWith("oy") -> { out.append("ɔʏ"); i += 2 }
                startsWith("au") -> { out.append("aʊ"); i += 2 }
                startsWith("ie") -> { out.append("iː"); i += 2 }
                startsWith("aa") -> { out.append("aː"); i += 2 }
                startsWith("ee") -> { out.append("eː"); i += 2 }
                startsWith("oo") -> { out.append("oː"); i += 2 }
                ...
                startsWith("er") && hasVowelBefore() && !isVowel(at(2)) -> { out.append("ɐ"); i += 2 }
                ch == 'e' && hasVowelBefore() && (i + 1 >= n || (at(1)?.let { it in "nlm" } == true && !isVowel(at(2)))) -> { out.append("ə"); i += 1 }
                ch == 'y' && i + 1 >= n -> { out.append("i"); i += 1 }
