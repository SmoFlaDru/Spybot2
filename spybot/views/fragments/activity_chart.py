...
def activity_chart_data(request):
    ...
    options, time_span, time_span_text = generate_options(
        allowed_time_spans,
        lambda n: f"Last {n} days",
        request.GET.get("timespan", None),
    )
...
def fragment(request):
    return render(
        request, "spybot/home/activity_fragment.html", activity_chart_data(request)
    )
